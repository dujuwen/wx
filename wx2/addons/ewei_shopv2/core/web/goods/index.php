...
class Index_EweiShopV2Page extends WebPage
{
	...
	public function change()
	{
		...
		if (!in_array($type, array('title', 'shorttitle','marketprice', 'total', 'goodssn', 'productsn', 'displayorder'))) {
			show_json(0, array('message' => '参数错误'));
		}
	...
	public function query()
	{
		...
		$ds = pdo_fetchall("SELECT id,title,shorttitle,thumb,marketprice,productprice,share_title,share_icon,description,minprice,costprice,total,content\n              FROM " . tablename('ewei_shop_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
