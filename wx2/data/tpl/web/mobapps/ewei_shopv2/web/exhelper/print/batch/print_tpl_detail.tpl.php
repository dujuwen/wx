<?php defined('IN_IA') or exit('Access Denied');?><span class="help-block">提示: 自提订单只可在批量打印中打印发货单，数据量大可造成页面卡顿，为保证稳定性单次打印的订单不要超过20张(可根据下单时间分批次打印)。</span>
<div class="panel panel-default">
	...
	<div id="result-order" style="padding: 5px;" class="panel-body">
		<?php  if(count($orders)>0) { ?>
			...
			<?php  if(is_array($orders)) { foreach($orders as $item) { ?> 
				<table class="table table-responsive table-order" style="table-layout: fixed; border-bottom: 1px solid #f2f2f2;  margin-bottom: 0px;" data-ordersn="<?php  echo $item['ordersn'];?>">
					<tr style="height: 0;">
					...
					<tr class="trorder">
						...
						<td colspan="4">
							...
							<?php  if(!empty($item['refundstate'])) { ?><label class='label label-danger'>退款申请</label><?php  } ?>
							<?php  if($item['dispatchtype']==1) { ?><label class='label label-primary'>用户自提</label><?php  } ?> 
							<?php  if($item['status']==2) { ?>
								快递公司: <?php  if(empty($item['expresscom'])) { ?>其他快递<?php  } else { ?><?php  echo $item['expresscom'];?><?php  } ?> 快递单号: <?php  echo $item['expresssn'];?>
							<?php  } ?>
						</td>
						<td class="aops" style="text-align:right;font-size:12px;" colspan="4">
							...
							<a class="op order-print-state-express" href="javascript:;">
								<?php  if($item['printstate']==0) { ?>快递单未打印<?php  } else if($item['printstate']==1) { ?>快递单部分打印<?php  } else if($item['printstate']==2) { ?>快递单打印完成<?php  } ?>
							</a>
							<a class="op order-print-state-invoice" href="javascript:;">
								<?php  if($item['printstate2']==0) { ?>发货单未打印<?php  } else if($item['printstate2']==1) { ?>发货单部分打印<?php  } else if($item['printstate2']==2) { ?>发货单打印完成<?php  } ?>
							</a>
			...
			<?php  if(is_array($item['goods'])) { foreach($item['goods'] as $k => $g) { ?>
				...
				<tr class="trbody trfold print-data" 
					data-send-status="<?php  echo $item['send_status'];?>" 
					data-print-goodid="<?php  echo $g['id'];?>" 
					data-print-total="<?php  echo $g['total'];?>" 
					data-print-title="<?php  echo $g['title'];?>" 
					data-print-option="<?php  echo $g['optiontitle'];?>" 
					data-print-orderid="<?php  echo $item['id'];?>" 
					data-print-ordersn="<?php  echo $item['ordersn'];?>" 
					data-print-goodssn="<?php  echo $g['goodssn'];?>" 
					data-print-productsn="<?php  echo $g['productsn'];?>" 
					...
					data-print-unit="<?php  echo $g['unit'];?>" 
					data-print-weight="<?php  echo $g['weight'];?>"
					>
					<td style="width: 26px; padding-left: 6px;">
					...
					<td style="text-align: left;overflow:hidden;">
						<span class="print-short" data-print-short="<?php  echo $g['shorttitle'];?>" data-toggle='ajaxEdit' data-href="<?php  echo webUrl('exhelper/short/edit',array('id'=>$g['id']))?>">
							<?php  if(empty($g['shorttitle'])) { ?>
								<?php  echo $g['title'];?>
							<?php  } else { ?>
								<?php  echo $g['shorttitle'];?>
							<?php  } ?>
						</span>
						
						<?php  if(!empty($g['optiontitle'])) { ?><br/><?php  echo $g['optiontitle'];?><?php  } ?>
						<?php  if(!empty($g['goodssn'])) { ?><br/><?php  echo $g['goodssn'];?><?php  } ?>
					</td>
					...
					<?php  if($k==0) { ?>
						...
						<td style="text-align:center" rowspan="<?php  echo count($item['goods'])?>">￥<?php  echo $item['price'];?> 
						...
						<td style="line-height:20px;text-align:center" class="ops" rowspan="<?php  echo count($item['goods'])?>">
							<label class='label label-<?php  echo $item['statuscss'];?> label-oss'><?php  echo $item['statusname'];?></label>
						</td>
					<?php  } ?>
					<td style="line-height:20px;text-align:center" class="ops">
						<label class='label label-default print-state-express' data-state="<?php  echo $g['printstate'];?>">快递单x<?php  echo $g['printstate'];?></label><br>
						<label class='label label-default print-state-invoice' data-state="<?php  echo $g['printstate2'];?>">发货单x<?php  echo $g['printstate2'];?></label>
					</td>
				</tr>
			<?php  } } ?>
				<tr class="trfold">
					<td class="address" colspan="9" style="text-align: right;">
						<?php  if($item['dispatchtype']==0) { ?>
							收件人: 
							<span class="print-data-realname" data-value="<?php  echo $item['address']['realname'];?>"><?php  echo $item['address']['realname'];?></span>
							(<span class="print-data-nickname" data-value="<?php  echo $item['address']['nickname'];?>"><?php  echo $item['address']['nickname'];?></span>) /
							<span class="print-data-mobile" data-value="<?php  echo $item['address']['mobile'];?>"><?php  echo $item['address']['mobile'];?></span>
							收货地址: 
							<span class="print-data-province" data-value="<?php  echo $item['address']['province'];?>"><?php  echo $item['address']['province'];?></span>
							<span class="print-data-city" data-value="<?php  echo $item['address']['city'];?>"><?php  echo $item['address']['city'];?></span>
							<span class="print-data-area" data-value="<?php  echo $item['address']['area'];?>"><?php  echo $item['address']['area'];?></span>
							<span class="print-data-address" data-value="<?php  echo $item['address']['address'];?>"><?php  echo $item['address']['address'];?></span>
				...
				<tr class="trfold">
					<td colspan="5" style="color: #F27474; text-align: left;">买家备注: <span class="remark1"><?php  if(!empty($item['remark'])) { ?><?php  echo $item['remark'];?><?php  } else { ?>无<?php  } ?></span></td>
					<td colspan="4" style="color:#F0C49B; text-align: right;">卖家备注: <span class="remark2"><?php  if(!empty($item['remarksaler'])) { ?><?php  echo $item['remarksaler'];?><?php  } else { ?>无<?php  } ?></span></td>
				</tr>
				</table>
			<?php  } } ?>
		...
		<div class="print-btn">
			<?php if(cv('exhelper.print.batch.express')) { ?>
				<a href="javascript:;" class="btn btn-success btn-sm" id="print-express" data-ban = "0" data-cate="1">打印快递单</a>
			<?php  } ?>
			<?php if(cv('exhelper.print.batch.invoice')) { ?>
				<a href="javascript:;" class="btn btn-warning btn-sm" id="print-invoice" data-ban = "0" data-cate="2">打印发货单</a>
			<?php  } ?>
			<?php if(cv('exhelper.print.batch.dosend')) { ?>
				<a href="javascript:;" class="btn btn-primary btn-sm" id="print-send" data-ban = "0">一键发货</a>
			<?php  } ?>
			<?php if(cv('exhelper.print.batch.express|exhelper.print.batch.invoice|exhelper.print.batch.dosend')) { ?>
				<a class="btn btn-default btn-sm" id="print-template" data-ban = "0" onclick='$("#chooseTemp").modal();'>选择打印模板</a>
			<?php  } ?>
		</div>
<?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('exhelper/print/print_tpl_template', TEMPLATE_INCLUDEPATH)) : (include template('exhelper/print/print_tpl_template', TEMPLATE_INCLUDEPATH));?> 
<script>
