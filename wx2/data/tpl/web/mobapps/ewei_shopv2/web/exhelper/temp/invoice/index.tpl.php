...
<div class="page-heading">
	...
	<h2>发货单模板管理 <small>数量: <span class='text-danger'><?php  echo $total;?></span> 条</small></h2> 
</div>
			...
			<?php  if(is_array($list)) { foreach($list as $row) { ?>
				<tr>
					...
					<td>
						...
						<a class='btn btn-default btn-sm' href="<?php  echo webUrl('exhelper/temp/invoice/edit',array('id' => $row['id']));?>"><i class='fa fa-edit'></i> 编辑</a> <?php  } ?>
						 <?php if(cv('exhelper.temp.invoice.delete')) { ?>
						 <a class='btn btn-default btn-sm'
						data-toggle='ajaxRemove' href="<?php  echo webUrl('exhelper/temp/invoice/delete',array('id' => $row['id']));?>" data-confirm="确定要删除该模板吗？"><i class='fa fa-trash'></i> 删除</a><?php  } ?>
					</td>
				</tr>
			<?php  } } ?>
		</tbody>
